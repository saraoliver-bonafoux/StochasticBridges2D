...
!
! A bridge is a trajectory conditioned to start at (x(0), y(0)) = (x0, y0) and end at (x(tT), y(tT)) = (xT, yT).
...
! - tT      : time duration of the bridges
! - (xT, yT): target state
! - (x0, y0): initial state
! - C       : constant used in the acceptance probability
! - R       : radius of the neighbourhoods around the initial and target states used to define entry/exit
...
! They represent transitions from the pluripotent state (x0, y0) to the differentiated state (xT, yT).
!
! -----------------------------------------------------------------------------------------
! SDEs parameters
! -----------------------------------------------------------------------------------------
...
! -----------------------------------------------------------------------------------------
! The bridge-sampling algorithm requires the quasi-stationary probability distribution QSD(x,y). 
...
! - filename_QSD             : file containing QSD(x,y) on an Ny x Nx grid
!
! In this example, we use the file "QSD_geneticswitch_Continuous.txt". 
! It corresponds to the genetic toggle switch model under demographic noise with intensity D = 0.0025.
! The distribution describes the metastable state (x0,y0) (pluripotent state) from which we want to sample
! transition paths.
! In the repository "https://github.com/saraoliver-bonafoux/FokkerPlanck2D", we provide a code to numerically integrate
! the Fokker-Planck equation in 2D.
!
! -----------------------------------------------------------------------------------------
! Output
! -----------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------------------------------
program Bridges2D_Continuous
	implicit double precision(a-h,o-z)
	!!! Bridge parameters
	integer, parameter :: M = 10
	double precision, parameter :: dt = 0.0005, tT = 30, xT = 0.0039216, yT = 1.99608, x0 = 1, y0 = 1, C = 0.5, R = 0.20
	!!! SDEs parameters 
	double precision, parameter :: D = 0.0025
	double precision, parameter :: a = 1.0, b = 1.0, k = 1.0, S = 0.5, n = 4, Sn = S**n ! Parameters of the genetic toggle switch model
	!!! Quasi-stationary distribution parameters
	double precision, parameter :: xmin = 0, xmax = 3, ymin = 0, ymax = 3
	integer, parameter :: Nx = 3001, Ny = 3001
	character(len=40) :: filename_QSD = "QSD_geneticswitch_Continuous.txt" 
	!!! General definitions
	double precision, allocatable :: x_bridge(:), y_bridge(:), t_bridge(:), QSD(:,:), xvec(:), yvec(:)
	...
	!-------------------------------------------------------------------------------------
 	call import_QSD(xmin, xmax, ymin, ymax, Nx, Ny, filename_QSD, QSD, xvec, yvec)
	...
	do ijk = 1, M
		...
		! This is the current state (x(t+dt), y(t+dt))
		!---------------------------------------------- 
		tdt = tT
		xtdt = xT
		ytdt = yT
		Pxtdtytdt = zxy_bilinear_interp(xtdt, ytdt, Nx, Ny, xvec, yvec, QSD)
		...
		do while (counter < Nsteps)
			...
			!---------------------------------------------- 
			x = sample_gaussian(xtdt - Fx(xtdt, ytdt) * dt, Gx(xtdt, ytdt) * sqrtDdt) ! Proposal for x(t)
			y = sample_gaussian(ytdt - Fy(xtdt, ytdt) * dt, Gy(xtdt, ytdt) * sqrtDdt) ! Proposal for y(t)
			
			if (x.ge.xmin .and. x.le.xmax .and. y.ge.ymin .and. y.le.ymax) then ! Only evaluate proposals lying inside the domain
				!----------------------------------------------
				! Evaluation of the proposal
				!---------------------------------------------- 
				Pxy = zxy_bilinear_interp(x, y, Nx, Ny, xvec, yvec, QSD)
				Pratio = Pxy / Pxtdtytdt
				Gxratio = gaussian(xtdt, x + Fx(x, y) * dt, Gx(x, y) * sqrtDdt) / gaussian(x, xtdt - Fx(xtdt, ytdt) * dt,  Gx(xtdt, ytdt) * sqrtDdt)
				Gyratio = gaussian(ytdt, y + Fy(x, y) * dt, Gy(x, y) * sqrtDdt) / gaussian(y, ytdt - Fy(xtdt, ytdt) * dt,  Gy(xtdt, ytdt) * sqrtDdt)
				H = C * Gxratio * Gyratio * Pratio ! Acceptance probability
				if (H.ge.1) print *, "Warning: H >= 1. Try reducing C. H =", H
				if (dran_u().lt.H) then ! dran_u(): generates a float random number in the interval [0,1)
					!----------------------------------------------
...
contains
	...
	double precision function Gx(x, y) ! Noise function in the x-direction
        double precision, intent(in) :: x, y
        Gx = sqrt(a * x**n / (Sn + x**n) + b * Sn / (Sn + y**n) + k * x)
	end function Gx
	
	double precision function Gy(x, y) ! Noise function in the y-direction
        double precision, intent(in) :: x, y
        Gy = sqrt(a * y**n / (Sn + y**n) + b * Sn / (Sn + x**n) + k * y)
	end function Gy
	...
	double precision function sample_gaussian(mu, sigma) 
		...
		sample_gaussian = sigma * dran_gbmw() + mu ! dran_gbmw(): generates a Gaussian random variable of mean 0 and variance 1
	...
	subroutine import_QSD(xmin, xmax, ymin, ymax, Nx, Ny, filename_QSD, QSD, xvec, yvec)
		implicit double precision(a-h,o-z)
    	double precision, intent (in) :: xmin, xmax, ymin, ymax
    	integer, intent (in) :: Nx, Ny
    	double precision, allocatable, intent(out) :: QSD(:,:), xvec(:), yvec(:)
		character(len=*), intent(in) :: filename_QSD 

		allocate(QSD(Ny, Nx), xvec(Nx), yvec(Ny))
		
		dx = (xmax - xmin) / dble(Nx - 1) ! Spatial resolution in the x-direction 
    	dy = (ymax - ymin) / dble(Ny - 1) ! Spatial resolution in the y-direction 
    	
		open(unit = 10, file = filename_QSD, status = "old", action = "read")
	...
	end subroutine import_QSD


end program Bridges2D_Continuous